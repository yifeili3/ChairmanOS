...
int screen_x[TERMINAL_NUM];
int screen_y[TERMINAL_NUM];

/* predetermined video mem physical buffer */
uint32_t video_mem_phys_addr[TERMINAL_NUM];

int32_t printf(int8_t *format, ...);
void putc(uint8_t c, uint32_t disp_terminal_on); //modified putc, adding scrolling
int32_t puts(int8_t *s);
int8_t *itoa(uint32_t value, int8_t* buf, int32_t radix);
int8_t *strrev(int8_t* s);
uint32_t strlen(const int8_t* s);

//clear screen and move cursor to (0,0) in clear()
void clear(void);
void test_interrupts(void);

void set_cursor(int row, int col); //helper function to set up cursor position
void backspace();//helper function to delete character in video memory
